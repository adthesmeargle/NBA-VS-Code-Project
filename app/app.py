...
### Check Google Drive for the process to get the mvp_0126, mvp_0226, mvp_0326, mvp_0426 df
mvp_0426_df = pd.read_csv(os.path.join(os.path.dirname(__file__),'../app' + '/mvp_0426.csv'))
mvp_0326_df = pd.read_csv(os.path.join(os.path.dirname(__file__),'../app' + '/mvp_0326.csv'))
...
mvp_0426_df["headshot_path"] = mvp_0426_df["headshot_path"].apply(resolve_relative_path)
mvp_0326_df["headshot_path"] = mvp_0326_df["headshot_path"].apply(resolve_relative_path)
...
mvp_0426_df["headshot_image"] = mvp_0426_df["headshot_path"].apply(lambda path: Image.open(path) if os.path.exists(path) else None)
mvp_0326_df["headshot_image"] = mvp_0326_df["headshot_path"].apply(lambda path: Image.open(path) if os.path.exists(path) else None)
...
if "page" not in st.session_state:
    st.session_state.page = "apr 2026"
...
st.sidebar.button("Apr 2026", on_click=go_to, args=("apr 2026",))
st.sidebar.button("Mar 2026", on_click=go_to, args=("mar 2026",))
...
# Render page content
if st.session_state.page == "apr 2026":
    st.title("April MVP Predictions 2026")
    ...
    # Render each row
    for _, row in mvp_0426_df.iterrows():
...
elif st.session_state.page == "mar 2026":
    st.title("March MVP Predictions 2026")
    ...
    # Render each row
    for _, row in mvp_0326_df.iterrows():
        cols = st.columns([1, 2, 1, 2])
        if row["headshot_image"]:
            cols[0].image(row["headshot_image"], width=80)
        else:
...
elif st.session_state.page == "feb 2026":
